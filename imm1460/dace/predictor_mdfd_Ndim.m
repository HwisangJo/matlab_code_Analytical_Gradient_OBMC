function  [y, or1, or2, dmse] = predictor_mdfd_Ndim(x, dmodel,n_unknown)
...
% Last update August 26, 2002
  ...
  if  mx == 1  % one site only
    ...
    elseif nargout>1     
        ...
        if prod(char(dmodel.regr)=='regpoly1')
            [f, df] = regpoly1_unknown_Ndim(x,n_unknown);
        elseif prod(char(dmodel.regr)=='regpoly2')
            [f, df] = regpoly2_unknown_Ndim(x,n_unknown);
        end
        [r, dr] = corrgauss_unknown_Ndim(dmodel.theta, dx,n_unknown);
        r = reshape(r, m, mx);
    end
...
%       drr=zeros(m,mx*2);
        drr=zeros(m,mx*n_unknown);
      
      for ik=1:n_unknown %n-unknown_n:n
        drr(:,[ik:n_unknown:(mx-1)*n_unknown+ik])=reshape(dr(:,ik),m,mx);
      end
...
%       dyy=reshape(dy',n,mx)';
        dyy=reshape(dy',n_unknown,mx)';

      % Unscaled Jacobian
      or1 = dyy .* repmat(dmodel.Ysc(2, :)', 1, n_unknown) ./ repmat(dmodel.Ssc(2,nx-n_unknown+1:nx), q, 1);
      if q == 1
        % Gradient as a column vector
%         or1 = or1';
      end
