function  [r, dr] = corrgauss_unknown_Ndim(theta, d,n_unknown)
...
% n_unknown=2;
if  nargout > 1
  dr = repmat(-2*theta(n-n_unknown+1:n),m,1) .* d(:,n-n_unknown+1:n) .* repmat(r,1,n_unknown);
%   dr=[zeros(m,n-n_unknown) dr_];
end