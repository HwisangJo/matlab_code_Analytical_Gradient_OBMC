...
for iii=1%:2
ypdf_ini(:,iii)=pdf(dist_type_{type_num(nt)},xx(:,iii),X0_((iii-1)*2+1),X0_((iii-1)*2+2));
ypdf_cal(:,iii)=pdf(dist_type_{type_num(nt)},xx(:,iii),X((iii-1)*2+1),X((iii-1)*2+2));
if type_num_(:,iii)==3
    ypdf_ini(:,iii)=pdf(dist_type_{type_num(nt)},-xx(:,iii)+2*X0_((iii-1)*2+1),X0_((iii-1)*2+1),X0_((iii-1)*2+2));
    ypdf_cal(:,iii)=pdf(dist_type_{type_num(nt)},-xx(:,iii)+2*X((iii-1)*2+1),X((iii-1)*2+1),X((iii-1)*2+2));
end
end
...
set(gca,'fontsize', 15);
ytickangle(90)
xlabel('Youngs Modulus (Pa) - \theta_{1}','fontsize', 17)
...
% y_range1_=[-0.013  -0.005];
% y_range1=[-0.013  -0.005];
y_range1_=[0.005  0.013];
y_range1=[0.005  0.013];
...
ylim([0 800])
% grid on
legend([s2 s1 s3],{'Initial output PDF','Calibrated output PDF','Experimental data'},'Location','best','fontsize', 12)
set(gca,'fontname','times','fontsize', 15)  % Set it to times
xlabel('Deflection (m) - \it y_{1}','fontsize', 17)
ylabel('PDF','fontsize', 17)
axx = gca;
% axx.FontSize = 20;
